...
(function($){
  ...
  var Valtastic = {
    ...
    Form : function(element,options){
      ...
      // Returns true if the form is valid. All fields required must be non-empty
      var form_valid = function(){
		var ret = true;
		$element.find('li.required input, li.required textarea').each(function(e) {
      ...
      $element.submit(function(){
        if(form_valid()) {
          ...
          //focus on invalid...
		if($element.find('.error input').get(0)) {
			$element.find('.error input').get(0).focus();
		} else if($element.find('.error textarea').get(0)) {
          $element.find('.error textarea').get(0).focus();
        }
		return false;
		}
      });
    ...
    Input : function(input_element,options){
      ...
      var displayErrors = function(){
		...
		$input_element.closest('li').removeClass('valid').addClass('error');
		//$input_element.closest('li').append('<ul class="errors"><li>'+errors.join('</li><li>')+'</li></ul>');
      };
